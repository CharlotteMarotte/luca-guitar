    ...
    <div className="px-20 py-8 flex flex-wrap place-items-center lg:border-y lg:border-white">
      <section className="relative mx-auto w-full">
        <nav className="flex justify-between items-center text-white w-full">
          <p className="text-2xl italic font-heading font-sm mr-8 pr-8 lg:border-r lg:border-white">
            Luca de Michieli
          </p>
        ...
        <div
          ...
          } md:hidden bg-paarl transition-all duration-300 w-full py-4`}
        >
          <ul className="flex flex-col space-y-4 font-body">
            {menuItems.map(({ name, href }) => (
              <li key={name}>
                <a
                  className="text-white hover:text-gray-200 hover:bg-white hover:bg-opacity-20 rounded-lg block p-2"
                  href={href}
                >
                  {name}
                </a>
